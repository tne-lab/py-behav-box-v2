...
class FiveChoiceGUI(GUI):
    ...
    def __init__(self, ws, task):
        ...
        for i in range(5):
            npl = CircleLightElement(self.task_gui, 50 + i*(25+60), 60, 30, Colors.lightgray, Colors.darkgray, task.nose_poke_lights[i])
            self.np_lights.append(npl)
            npi = NosePokeElement(self.task_gui, 50 + i * (25 + 60), 150, 30, task.nose_pokes[i])
